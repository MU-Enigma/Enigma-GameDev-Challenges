...
func set_move(varCol, varRow):
	...
	check_display() # User-defined function
...
func get_rook_moves(piece_position : Vector2):
	...
	for direction in directions:
		...
		while is_valid_position(pos): # User-defined function
			if !move_undo_append_fidelity(4, piece_position, pos, _moves): break # User-defined function
			pos += direction
...
func get_bishop_moves(piece_position : Vector2):
	...
	for direction in directions:
		...
		pos += direction
		while is_valid_position(pos): # User-defined function
			if !move_undo_append_fidelity(3, piece_position, pos, _moves): break # User-defined function
			pos += direction
...
func get_queen_moves(piece_position : Vector2):
	...
	for direction in directions:
		...
		pos += direction
		while is_valid_position(pos): # User-defined function
			if !move_undo_append_fidelity(5, piece_position, pos, _moves): break # User-defined function
			pos += direction
...
func get_knight_moves(piece_position : Vector2):
	...
	for direction in directions:
		...
		if is_valid_position(pos): # User-defined function
			move_undo_append_fidelity(2, piece_position, pos, _moves) # User-defined function
...
func get_pawn_moves(piece_position : Vector2):
	...
	# Fidelity condition at the end...
	if is_valid_position(pos) && is_enemy(pos) && (board[pos.y][pos.x] != (-1 if white else 1)): # User-defined functions
...
# Used to determine the moves of the Rook, the Bishop, the Queen and the Knight
func move_undo_append_fidelity(num : int, start_pos : Vector2, end_pos : Vector2, local_moves : Array):
	num = num if white else -num
	if is_empty(end_pos): # User-defined function
		# Piece is moved to check if it leads to no CHECK and placed back
		board[end_pos.y][end_pos.x] = num
		board[start_pos.y][start_pos.x] = 0
		if (white && !is_in_check(white_king_pos)) || (!white && !is_in_check(black_king_pos)): # User-defined function
			local_moves.append(end_pos)
		board[end_pos.y][end_pos.x] = 0
		board[start_pos.y][start_pos.x] = num
	elif is_enemy(end_pos): # User-defined function
		# Piece captures to check if it leads to no CHECK and undoes the capture
		var temp = board[end_pos.y][end_pos.x]
		if temp != -num: # Condition of fidelity
			board[end_pos.y][end_pos.x] = num
			board[start_pos.y][start_pos.x] = 0
			if (white && !is_in_check(white_king_pos)) || (!white && !is_in_check(black_king_pos)): # User-defined function
				local_moves.append(end_pos)
			board[end_pos.y][end_pos.x] = temp
			board[start_pos.y][start_pos.x] = num
		return false
	else: return false # It's an ally (You can't capture it)
	return true
